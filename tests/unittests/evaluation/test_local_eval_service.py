...
from google.adk.evaluation.local_eval_service import AgentCreator
...
def mock_agent_creator():
  return mock.create_autospec(AgentCreator)
...
@pytest.fixture
def eval_service(mock_agent_creator, mock_eval_sets_manager):
...
async def test_perform_inference_success(
    eval_service, mock_agent_creator, mock_eval_sets_manager
):
  eval_set = EvalSet(
  ...
  mock_agent_creator.get_agent.return_value = mock_agent
  ...
  inference_request = InferenceRequest(
      app_name="test_app",
      eval_set_id="test_eval_set",
      inference_config=InferenceConfig(max_inference_parallelism=2),
  )
  ...
  mock_agent_creator.get_agent.assert_called_once()
...
async def test_perform_inference_with_case_ids(
  ...
  inference_request = InferenceRequest(
      ...
      inference_config=InferenceConfig(max_inference_parallelism=1),
  )
