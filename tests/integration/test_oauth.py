...
MOCK_OAUTH_REDIRECT_URI = "http://testserver/auth/callback"
...
async def test_auth_google_callback_success_and_session(
  ...
  callback_response = test_app_client.get(
      f"/auth/callback?code={MOCK_AUTH_CODE}&state={MOCK_OAUTH_STATE}",
  ...
  # 5. Verify session by accessing a protected route (/test-auth-check)
...
async def test_logout_clears_session_and_redirects(test_app_client: TestClient):
  ...
  with (
    ...
    test_app_client.get(
        "/auth/callback?code=anycode&state=login_for_logout_state",
  ...
  # /test-auth-check should now redirect to /login
