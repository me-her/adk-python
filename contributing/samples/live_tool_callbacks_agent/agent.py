...
def before_tool_security_callback(
  ...
  if tool.name == "get_weather" and args.get("location", "").lower() in ["classified", "secret"]:
    print(f"🚫 SECURITY: Blocked weather request for restricted location: {args.get('location')}")
    return {
      "error": "Access denied",
...
async def before_tool_async_callback(
  ...
  if tool.name == "calculate_async" and args.get("operation") == "divide" and args.get("y") == 0:
...
def after_tool_enhancement_callback(
    tool, args: Dict[str, Any], tool_context: ToolContext, tool_response: Dict[str, Any]
) -> Optional[Dict[str, Any]]:
...
async def after_tool_async_callback(
  ...
  print(f"🔄 ASYNC AFTER: Post-processing response from '{tool.name}' asynchronously")
